...
public class Room {
    ...
    public boolean IsItemOnXY(int X, int Y)
    {
        ...
        {
            RoomItem Item = (RoomItem)reader.next();
            if(Item.X == X && Item.Y == Y && !Item.GetBaseItem().CanWalk)
                return true;
            else {
                Map<Integer, AffectedTile> AffectedTiles = GetAffectedTiles(Item.GetBaseItem().Lenght, Item.GetBaseItem().Width, Item.X, Item.Y, Item.Rot);
                ...
                while(reader2.hasNext())
                {
                    ...
                    if(Item.GetBaseItem().CanWalk && Item.X == Tile.X() && Item.Y == Tile.Y())
                    {
                        if(Item.GetBaseItem().Interactor == "gate" && Item.ExtraData == "1")
                            return false;
                        else
                            return true;
                    }
